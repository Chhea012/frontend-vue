      ...
      <div class="bg-white p-6 rounded-lg shadow">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-800">{{ libraryStore.borrowedBooks.length }}</p>
          </div>
        </div>
      </div>
      <div class="bg-white p-6 rounded-lg shadow">
    ...
    <div class="bg-white rounded-lg shadow">
      ...
      <div class="p-6">
        ...
        <div v-else class="space-y-4">
          <div v-for="borrow in libraryStore.borrowedBooks.slice(0, 5)" :key="borrow.id" class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            <div>
              <p class="font-medium text-gray-800">{{ libraryStore.getBookTitle(borrow.bookId) }}</p>
              <p class="text-sm text-gray-600">Borrowed by {{ libraryStore.getMemberName(borrow.memberId) }}</p>
            </div>
            <div class="text-sm text-gray-500">
              {{ libraryStore.formatDate(borrow.borrowDate) }}
            </div>
          </div>
        </div>
      </div>
